...
- Collision maps are flat arrays (144 values) parsed into 16-column 2D grids via `parse2D()` in utils.js
...
- Doors navigate via `redirectTo()` (LinkedIn, GitHub, Behance) after a 1000ms delay
...
## IMPORTANT
- `utils.js` provides `parse2D(arr)` and `createObjectsFrom2D(arr)` as standalone functions. Do NOT add `Array.prototype` modifications.
